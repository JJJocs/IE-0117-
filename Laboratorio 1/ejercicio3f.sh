...
generate_report() {
    ...
    if [ -n "$mode" ] && [ -n "$date" ]; then
        ...
        grep "\[$mode\]" "$nombreDocumento" | grep "ERROR" | while read -r line; do
            error_date=$(echo "$line" | awk '{print $1}')
            error_time=$(echo "$line" | awk '{print $2}')
            error_description=$(echo "$line" | awk -F '] ' '{print $NF}')
            echo "Fecha: $error_date"
        ...
        grep "^$date" "$nombreDocumento" | grep "ERROR" | while read -r line; do
            error_date=$(echo "$line" | awk '{print $1}')
            error_time=$(echo "$line" | awk '{print $2}')
            error_description=$(echo "$line" | awk -F '] ' '{print $NF}')
            echo "Fecha: $error_date"
...
main() {
    ...
    # Solicitar el nombre del documento solo si no se ingresó la opción -h
    if [ "$mode" != "h" ]; then
